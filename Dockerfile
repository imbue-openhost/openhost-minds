...
# Dockerfile changes, mirror the change here.
...
# Copy the whole app repo (including .git and .git/modules -- see
# .dockerignore) and assemble the workspace at /mngr/code from the template
# subtree, with the template's real git history reconstituted from the
# submodule and the deployed template SHA stamped at a stable image-layer
# path. That history is what lets update-self merge a newer deployed template
# into an existing workspace (see the prepare script).
...
RUN chmod +x /usr/local/bin/openhost-minds-prepare-workspace \
    && openhost-minds-prepare-workspace /openhost_build_ctx /mngr/code /opt/openhost-template-version \
    && rm -rf /openhost_build_ctx

# Build the workspace from full source.
RUN bash /mngr/code/scripts/build_workspace.sh

# Move the baked workspace off the volume mount path so the shipped image has
# /mngr/code/ EMPTY: at runtime /mngr/ is a persistent volume mount, and the
# first-boot seed relocates /docker_build_code onto it.
